...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // Used Regular Expression approach to complete this task   
  // Create RegEx expression
  std::regex re("[^0-9A-Z+$]\\s*(and|or)\\s*[\\'\"]{0,1}", std::regex_constants::icase);

  // DETECT SQL INJECTION PATTERN MATCH
  if (std::regex_search(sql, re)) {
      // Display the error message
      std::cout << "Failure! SQL INJECTION DETECTED !!!" << std::endl;
      return false;
  }
...
int main()
{
  ...
  // Used to stop SQLite for 10 seconds to view console.
  sqlite3_sleep(10000);
